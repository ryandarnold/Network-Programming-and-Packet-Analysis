...
```bash
sudo python3 traceroute.py <destination> [arguments]
```
...
```bash
sudo python3 traceroute.py www.google.com -q 5
```

Print numeric IP addresses only:

```bash
sudo python3 traceroute.py www.google.com -n
```

Print unanswered probe summary:

```bash
sudo python3 traceroute.py www.google.com -S
```
